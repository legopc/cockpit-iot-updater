...
#   podman save --format docker-archive IMAGE -o base-export.tar
# base_sha256 MUST be the sha256 of a docker-archive format tar so it matches
# what the appliance will compute at apply time.
if [[ -n "$BASE_IMAGE" ]]; then
    echo "Exporting base image (podman save docker-archive): ${BASE_IMAGE}"
    echo "  (This may take a few minutes for a large image…)"
    podman save --format docker-archive "${BASE_IMAGE}" -o "${BASE_TAR}" || {
...
if [[ -n "$TARGET_IMAGE" ]]; then
    echo "Exporting target image (podman save docker-archive): ${TARGET_IMAGE}"
    echo "  (This may take a few minutes for a large image…)"
    podman save --format docker-archive "${TARGET_IMAGE}" -o "${TARGET_TAR}" || {
